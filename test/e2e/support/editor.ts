...
/**
 * Locate an app-menu button by its plugin `id` (the `id` of the `defineMenuButton`
 * definition, surfaced as `data-test="app-menu-button-<id>"`), e.g. `publish`,
 * `revert`, `import_existing`, `translations`, `exit`. Returns a Locator, so
 * callers can assert `toBeDisabled()` / `toBeEnabled()` or click it.
 */
