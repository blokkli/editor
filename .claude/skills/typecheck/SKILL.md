...
- **`src/build/**`**, **`src/module.ts`**, **`src/module/**`** → `npm run typecheck:build`
- **`src/modules/*/runtime/server/**`** → `npm run typecheck:server`
- **`src/modules/*/runtime/**`** (non-server) → `npm run typecheck:runtime`
- **`src/modules/*/index.ts`**, **`src/modules/*/build/**`** → `npm run typecheck:build`
...
- The runtime typecheck is typically the most useful one during development since
  most editor code lives in `src/runtime/`
