...
- Pressing the space bar while hovering the canvas now activates the move/pan cursor, matching common design tool conventions.
- After duplicating blocks, the duplicated blocks are automatically selected and scrolled into view.
- After deleting a block, the page scrolls to bring the nearest remaining block into view.
- The dragging overlay label now shows the nesting trail (e.g. parent block name) and stays visible at viewport edges.
- The analyze category filter is hidden when there is only one category.
- The app menu now scrolls correctly when the list of items is longer than the visible area.
